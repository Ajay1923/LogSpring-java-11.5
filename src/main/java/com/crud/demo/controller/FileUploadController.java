...
public class FileUploadController {
    ...
    public String allUsers(Model model, HttpSession httpSession) {
        ...
        if (userOptional.isPresent()) {
            ...
            String accessLevel = user.getAccess(); // Check access level

            boolean isAdmin = "admin".equalsIgnoreCase(accessLevel);
            model.addAttribute("isAdmin", isAdmin); // Pass the admin status to the model

            if (isAdmin) {
                // Fetch all users from the repository for admin users
                List<User> users = userRepository.findAll();
                model.addAttribute("users", users);
                return "allusers"; // Return the allusers.html page
            }
        }

        // If the user is not an admin, handle access denial
        model.addAttribute("errorMessage", "You do not have permission to view all users.");
        return "settings"; // Redirect to a settings or access denied page
    }
